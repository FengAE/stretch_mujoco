"""Open one of the ten generated open-plan office scenes."""
...
@click.option("--auto-orbit", is_flag=True, help="Continuously orbit around the office.")
...
def main(scene: int, top_view: bool, auto_orbit: bool, orbit_speed: float, headless: bool) -> None:
    """Interactively inspect a generated textured office scene."""
    catalog = json.loads((SCENE_ROOT / "catalog.json").read_text())
    paths = [SCENE_ROOT / entry["mjcf"] for entry in catalog["scenes"]]
    if len(paths) != 10:
        raise click.ClickException(
            f"Expected 10 generated scenes in {SCENE_ROOT}, found {len(paths)}"
        )
    path = paths[scene - 1]
    ...
    click.echo(
        f"Loaded {path.name}: {model.ntex} textures, {model.nmat} materials, "
        f"{model.nmesh} meshes, {model.ngeom} geoms"
    )
    if headless:
        return
