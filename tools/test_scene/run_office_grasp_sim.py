...
``<scene_id>_*_grasp_site.json`` with ``"reached": true``, this script:
  ...
  6. Executes pregrasp -> align -> close -> contact check -> attach -> retreat
     -> lift, and judges success from physical contact + lift height.
...
def _boost_friction(model: mujoco.MjModel, body_names: list[str], friction: float) -> int:
    """Raise sliding friction on every geom in the named bodies' subtrees.

    Mirrors ``collect_random_object_grasp_episodes.py::build_episode_model``.
    """
    changed = 0
...
def run_object(
    ...
    try:
        ...
        sim.move_to(Actuators.wrist_pitch, float(approach_joints[4]))
        sim.move_to(Actuators.wrist_roll, float(approach_joints[5]))
        ...
        sim.move_to(Actuators.wrist_pitch, float(joints[4]))
        sim.move_to(Actuators.wrist_roll, float(joints[5]))
        _wait(sim, Actuators.wrist_yaw, timeout=20.0)
        _wait(sim, Actuators.wrist_pitch, timeout=20.0)
        _wait(sim, Actuators.wrist_roll, timeout=20.0)
        ...
        result.update(
            ...
            pregrasp_wrist_pitch_error=float(joints[4]) - float(status.wrist_pitch.pos),
            pregrasp_wrist_roll_error=float(joints[5]) - float(status.wrist_roll.pos),
        ...
        _wait(sim, Actuators.wrist_pitch, timeout=30.0)
        _wait(sim, Actuators.wrist_roll, timeout=30.0)
